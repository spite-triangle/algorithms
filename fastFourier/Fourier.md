...
**存在条件：**
$$
\int_{-\infty}^{\infty} |f(t)| dt < \infty \tag{3.2}
$$
...
* 序列傅里叶变换
    ...
    $$
    X(e^{j \omega}) = \sum_{n=-\infin}^{\infin} x(n) e^{-j \omega n} \tag{5.1}
    $$
...
&emsp;&emsp;**对于序列傅里叶变换而言： $\Omega T = \omega$, $T = \frac{1}{Fs}$为信号的采样周期。对于离散傅里叶变换而言：可以将 $\omega = \frac{2 \pi}{N} k$。整理两个方程，就能将离散傅里叶变换的频率变量 $k$ 换算为真实的角频率 $\Omega$。**

$$
\Omega = \frac{\omega}{T} = k 2 \pi \frac{Fs}{N} \tag{5.3} 
$$
